...
def calc_inversion_signals(c, kama_arr, atr_arr, obv_arr):
    ...
    # Valido solo se KAMA si e' appiattita (slope < 2% in 10 giorni).
    # Se KAMA scende ancora, OBV divergenza e' prematura — troppi falsi positivi.
    obv_divergence = False
    if n > 20:
        price_trend = float(c[-1] - c[-20])
        obv_trend   = float(obv_arr[-1] - obv_arr[-20])
        obv_div_raw = bool(price_trend < 0 and obv_trend > 0)
        # Controlla appiattimento KAMA ultimi 10 giorni
        kama_flat = False
        if n >= 11 and not np.isnan(kama_arr[-1]) and not np.isnan(kama_arr[-11]):
            k_now  = float(kama_arr[-1])
            k_10ag = float(kama_arr[-11])
            if k_10ag != 0:
                kama_slope = abs(k_now - k_10ag) / abs(k_10ag)
                kama_flat  = bool(kama_slope < 0.02)  # meno del 2% in 10 giorni
...
def compute_top20(results):
    ...
    Richiede dist_52w_low <= 30% — esclude ETF lontani dal minimo.
    """
    # Categorie escluse dal TOP 20 operativo:
    # - Short: logica inversa, ATR rising = mercato in caduta = segnale bearish
    ...
    candidates = [
        ...
        and r.get('dist_52w_low', 999) <= 30.0   # max 30% sopra il minimo 52W
